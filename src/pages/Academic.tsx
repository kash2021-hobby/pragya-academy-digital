...
// Memoized Principal Message Component
    ...
    <div className="flex flex-col md:flex-row gap-8">
      <div className="md:w-1/3 shrink-0">
        <img 
          src={principalImg} 
          alt="Principal" 
          className="rounded-xl shadow-md w-full max-w-[250px] mx-auto"
          loading="lazy"
          width="250"
          height="250"
        />
...
const Academic = () => {
  ...
  // Preload images on component mount for faster tab switching
  useEffect(() => {
    const preloadImages = [principalImg, advisorImg];
  ...
  const activeContent = useMemo(() => {
    return activeTab === 0 ? <PrincipalMessage /> : <AdvisorMessage />;
  }, [activeTab]);
          ...
          <div className="flex justify-center gap-2 mb-10">
            {tabs.map((tab, i) => (
              <button
                ...
                aria-label={`Switch to ${tab}'s Message`}
              >
                {tab}'s Message
              </button>
            ))}
